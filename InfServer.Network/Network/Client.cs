    ...
    {	// Member variables
        ...
        {	//Sync up!
            using (DdMonitor.Lock(_sync))
            {
                ...
                {	//Farewell~
                    Log.write(TLog.Normal, "Client timeout: {0}", _logger, this);
                    destroy();
                    //TODO: Make sure when players timeout that they are taken out of the arena/player list for other clients
                    //Needs testing to confirm this is the cause of duplicating names when they re-enter
                    return;
                }
        ...
        {	//No need to do this twice
            if (_bDestroyed)
            {
                Log.write(TLog.Error, "Attempted to destroy already destroyed Client '{0}'", _logger, _ipe);
                return;
            }
        ...
        {	//Check for data streams
            ...
            {	//Make sure the rid is correct
                if (info.rid == -1)
                {
                    Log.write(TLog.Error, "Reliable info was queued for sending with an unassigned reliable number.", _logger);
                    continue;
                }
        ...
        public void sendReliable(PacketBase packet, Action completionCallback, int streamID)
        {
            if (_bDestroyed)
            {
                Log.write(TLog.Error, "Attempted to send reliable packet to destroyed Client: {0}", _logger, this);
                return;
            }
        ...
        public override void send(PacketBase packet)
        {
            if (_bDestroyed)
            {
                Log.write(TLog.Error, "Attempted to send packet to destroyed Client: {0}", _logger, this);
                return;
            }
