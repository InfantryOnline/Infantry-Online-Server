...
namespace InfServer.Game
{
    ...
    {	// Member variables
        ...
        /// </sumary>
        public void cleanup()
        {   //Loop through each arena and boot each player in that arena.
            foreach (KeyValuePair<string, Arena> arena in _arenas.ToList())
            {
                foreach (Player p in arena.Value.Players.ToList())
                {
                    ...
                    //Disconnect them
                    p.disconnect(Disconnect.DisconnectReason.DisconnectReasonOtherSideTerminated);
                }
            }
