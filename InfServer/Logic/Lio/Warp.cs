...
{	// Logic_Game Class
	...
	{	/// <summary>
		...
		/// </summary>
		static public void Warp(Helpers.WarpMode mode, Player player, IEnumerable<LioInfo.WarpField> warpGroup)
		{	//Redirect
			Warp(mode, player, warpGroup, player._server._zoneConfig.vehicle.warpDamageIgnoreTime);
		}

		/// <summary>
		/// Handles an item pickup request from a client
		/// </summary>
		static public void Warp(Helpers.WarpMode mode, Player player, IEnumerable<LioInfo.WarpField> warpGroup, int invulnTime)
		{	//Search for valid warps to use
			...
			if (valid.Count == 1)
				Warp(mode, player, valid[0], invulnTime);
			else
				Warp(mode, player, valid[player._arena._rand.Next(0, valid.Count - 1)], invulnTime);
		}

		/// <summary>
		/// Handles an item pickup request from a client
		/// </summary>
		static public void Warp(Helpers.WarpMode mode, Player player, LioInfo.WarpField warp, int invulnTime)
		{	//Resolve our box
