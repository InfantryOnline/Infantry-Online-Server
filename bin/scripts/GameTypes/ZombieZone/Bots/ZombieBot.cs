...
namespace InfServer.Script.GameType_ZombieZone
{
	...
	{	// Member variables
		...
		{	//Look at the players on the target team
			...
			{	//Find the closest player
				...
				bool bClearPath = Helpers.calcBresenhemsPredicate(_arena,
					_state.positionX, _state.positionY, p._state.positionX, p._state.positionY,
		...
		{	//Is the team tight enough to warrant this check?
			Script_ZombieZone.TeamState state = zz.getTeamState(targetTeam);
			if (state != null)
				if (state.teamSeperation > Script_ZombieZone.c_zombieDistanceMaxSep)
					return true;
			
			//Get the average team location
			short posX, posY;

			Script_ZombieZone.averageTeamLocation(targetTeam, out posX, out posY);

			//Compare our distance
			return Helpers.isInRange(	Script_ZombieZone.c_zombieMaxRespawnDist + Script_ZombieZone.c_zombieDistanceLeeway, 
										_state.positionX, _state.positionY, posX, posY);
		}
		...
		private bool headToDistraction(Script_ZombieZone.ZombieDistraction distract)
		{
			...
			steering.steerDelegate = delegate(InfantryVehicle vehicle)
			{
				Vector3 wander = vehicle.SteerForWander(0.5f);
				Vector3 seek = vehicle.SteerForSeek(distractpos);
				return wander + (seek * 0.8f);
			};
