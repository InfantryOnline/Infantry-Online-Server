...
{       // Script Class
    ...
    public static class ArenaExtensions
    {
        ...
        {       //Sanity
            ...
            arena.itemSpawn(item, quantity, x, y, null);
        }
    ...
    public static class PlayerExtensions
    {
        ...
        */
        ...
        {       //Check each skill
            ...
            p.checkUpgradeSkill("Upgrade Energy Pistol");
        }
    ...
    {   ///////////////////////////////////////////////////
        ...
        public const int c_minResAmmo = 250;              //You get at least this much ammo when being ressed (or res cost, whichever is less)
        ...
        public const int c_ammoGenerateAmount = 100;                             //The amount of ammo generated each time
        public const int c_ammoGeneratePerLevel = 30;                   //The amount of extra ammo the generator generates per level
        ...
        public const float c_zombieInitialAmountPP = 2f;                //The amount of zombies per player initially spawned (minimum of 1)
        ...
        public const int c_ammoEaterDefaultPrize = 55;      //Default amount of ammo given to ammo eaters
        ...
        public class TeamState
        {
            ...
            public bool atMission;                              //Is the team at the mission objective?
            public bool atChopper = false;
            public int missionEnding = 0;                           //Determines what ending the team will experience
            public short missionX = 816;                              //Coords for mission ending objective
            public short missionY = 2048;
            public Helpers.ObjectState missionObjective;        //The actual objective's state
            public int tickAtObjective = 0;
        ...
        {       //Maintain our arena if the game is running!
            ...
            {   //Should we update the zombie count ticket?
                ...
                foreach (TeamState team in _states.Values)
                {
                    ...
                    if (team.supplyDropsFound >= 15) //10 being winning amount, set as variable
                    {   //Find out if they made it to their beacon
                        if (team.missionEnding == 1)
                        {   //Check for winners
                           
                            if (team.atChopper && team.atMission && Environment.TickCount - team.tickAtObjective > 75000)
                            {
                                _arena.sendArenaMessage("Team " + team.team._name + " has been rescued by the chopper!");
                                _arena.sendArenaMessage("Game Over!");
                                _arena.gameEnd();
                            }
                            if (team.atMission && Math.Abs(team.lastLocationX - team.missionX) < 100 && Math.Abs(team.lastLocationY - team.missionY) < 100)                                                        
                                team.atChopper = true;                            
                            else
                            {
                                team.tickAtObjective = Environment.TickCount;
                                team.atChopper = false;
                            }
                            foreach (Player p in team.originalPlayers)
                            {
 
                                if (!team.atMission && Math.Abs(p._state.positionX - team.missionX) < 100 && Math.Abs(p._state.positionY - team.missionY) < 100)
                                {
                                    team.missionObjective.positionZ = 200;
                                    team.missionObjective.positionX += 700;
                                    team.missionObjective.positionY += 300;
                                    _arena.newBot(typeof(Chopper), (ushort)127, p._team, null, team.missionObjective);
                                    _arena.sendArenaMessage("Arnolds flying a chopper to " + team.missionObjective.letterCoord() + " to rescue team " + p._team._name + "!!!");
                                    team.atMission = true;
                                    team.tickAtObjective = Environment.TickCount;
                                    break;
                                }
                        ...
                        //Start their random ending                        
                        if (team.missionEnding == 0)
                        {   //Mark them as having an ending
                            team.missionEnding = 1;
                            team.atMission = false;
                            VehInfo chopperLanding = _arena._server._assets.getVehicleByID(Convert.ToInt32(410));
                            Helpers.ObjectState chopperState = new Protocol.Helpers.ObjectState();
                            chopperState.positionX = team.missionX;
                            chopperState.positionY = team.missionY;
                            chopperState.positionZ = 0;
                            chopperState.yaw = 0;
                            team.missionObjective = chopperState;
                            //Spawn their beacon
                            _arena.newVehicle(
                                            chopperLanding,
                                            _arena.Teams.FirstOrDefault(), null,
                                           chopperState);
                            team.team.sendArenaMessage("Arnold: Get to the chopper at " + chopperState.letterCoord());
                        }
        ...
        {       //Let's get some class kits down!
            ...
            _arena.spawnItemInArea(AssetManager.Manager.getItemByName("Swordsman Kit"), 2, posX, posY, c_startAreaRadius);
        }
        ...
        public void ammoGenerate(TeamState state, Computer ammoGenerator)
        {
            ...
            if (amountRemaining > 0 && ammoArea < ammoAmount)
                _arena.itemSpawn(ammo, (ushort)amountRemaining, ammoGenerator._state.positionX, ammoGenerator._state.positionY, (short)range, null);
        }
        ...
        {       //Make sure we don't trigger this twice
            ...
            for (int i = 0; i < opener._team.ActivePlayerCount; ++i)
                _arena.spawnItemInArea(AssetManager.Manager.getItemByName("Ammo"), (ushort)_rand.Next(225, 275), posX, posY, c_supplyDropAreaRadius);
            ...
            _arena.spawnItemInArea(AssetManager.Manager.getItemByName("Energizer"), (ushort)_rand.Next(1, 3), posX, posY, c_supplyDropAreaRadius);
            ...
            if (_rand.Next(4) == 1)  //1/4 chance of spawning fuel cell
        ...
        {       //Find an open unblocked space in the map
            ...
            {
                team.sendArenaMessage("*Players should keep moving towards the drop box and stay with your team, or setup a base by buying equipment(turrets/etc) in F10.");
                team.sendArenaMessage("*Drop boxes give rewards, ammo, and weapons when opened; and are necessities in collecting money and supplies to build a strong base.");
                team.sendArenaMessage("*Money does not save from game to game, so make sure to use what the cash you earn each game.");
                team.sendArenaMessage("See the top-right corner of the game screen for drop box locations and other useful information.");
        ...
        public bool playerChatCommand(Player player, Player recipient, string command, string payload)
        {
            ...
            {
                player.sendMessage(0, "&Thanks for confirming that you're still here. Enjoy your game!");
                player.setVar("afk", -c_afkNumGamesValidated + 1);
            }
            ...
            {
                TeamState state = getTeamState(player._team);
                string teamGenMessage = state.ammoGenerator != null ? "Your team already has an ammo generator at " + state.ammoGenerator._state.letterCoord() : "Only one is allowed per team at a time.";
 
                player.sendMessage(0, String.Format("Ammo generators will deposit ammo directly into your inventory if you are within {0} distance.  Otherwise, or if you're already full, it will drop your share at its location.  They decay over time, and have a limited number of drops. {1}.", c_ammoGenerateDirectDist, teamGenMessage));
            }
        ...
        {       //Are they any kit items?
            ...
            else if (drop.item.name == "Swordsman Kit")
            {   //Only marines can change class
                if (player._baseVehicle._type.Id < 10 || player._baseVehicle._type.Id > 14)
                    return false;
 
                player.inventoryModify(AssetManager.Manager.getItemByName("convertRealToFakeSwordsman"), 99);
 
                player.upgradeVehicle(115);
            }
        ...
        public bool playerJoinGame(Player player)
        {
            ...
            {   //Make him a zombie!
                ...
                player.sendMessage(-1, "&The game is already in progress so you have been spawned as a zombie until next game.");
                return false;
            }
        ...
        {       //Game finished, perhaps start a new one
                ...
                if (_states.Count > 1)
                {
                    ...
                    if (victorState != null)
                        foreach (Player p in victorState.originalPlayers)
                        {
                            ...
                            p.syncState();
                            p.sendMessage(0, String.Format("&You've received {0} experience for being on the last surviving team.", winReward));
                        }
            ...
            foreach (Player p in _arena.Players)
            {
                ...
                else
                {
                    ...
                    if (newAFK > 0)
                    {
                        p.sendMessage(0, "=========(o'.')-o=GAME TIPS=o-('.'o)========");
                        ...
                        p.sendMessage(0, "Players must type ?afk, or keep moving to play. (A.K.A. ... MOVE IT!)");
                        p.sendMessage(0, "!Pay attention to your team's current drop box location and other useful information located in the top-right corner of the game screen.");
                        p.sendMessage(0, "!Upgrade your character by pressing F11 to bring up the skills menu. Upgrades in F11 require EXPERIENCE points to purchase them.");
                        p.sendMessage(0, "!After upgrading a certain weapon, make sure to purchase it from the store menu(F10). Also be sure to upgrade your character before the game starts.");
                        p.sendMessage(0, "==GAME WILL BE STARTING IN A FEW MOMENTS==");
                    }
        ...
        public static void suddenDeathWave(ZombieTransitions transitions, List<ZombieType> composition, int time = 15)
        {
            ...
            trans.zombieTickers[1] = info => info.time > 2 ? "Wipe the survivors out!" : "Until all that remains is Silence.";
        ...
        {
            ZombieTransitions.ZombieTransition trans = transitions.gracePeriod(30);
            ...
            trans.initialZombieCount = ZombieTransitions.constInt(5);
            trans.initialTime = 30;
            trans.spawnComposition = ZombieTransitions.constComposition((new List<ZombieType>()).addType(ZombieTransitions.AlienZombie, 6).addType(ZombieTransitions.HumanZombie, 2));
            trans.finalZombieCount = p => zombieAmount(p, 7);
            trans.finalTime = 10;
            ...
            trans.started = delegate(Script_ZombieZone.TeamState state)
            {
                state.team.sendArenaMessage("=======(>'.')>-GAME TIPS-<('.'<)=======");
                state.team.sendArenaMessage("Keep moving towards the next drop box location to collect supplies and cash. Buy turrets and other equipment in the store menu by pressing F10.");
                state.team.sendArenaMessage("The longer that you and your team stays in one location, or the farther that your team strays from eachother, the higher your team's awareness. The higher the awareness, the more zombies that spawn.");
                state.team.sendArenaMessage("!The zombies are coming! We need to get moving now! Keep track of your location, and the next drop box location(top-right corner of the game screen).");
                state.team.sendArenaMessage("=========================");
                state.team.sendArenaMessage("*=->!ENJOY YOUR GAME!<-=*");
            };
 
            transitions.gracePeriod(45);
 
            //adds some suicides, still relaxed
            trans = transitions.newTransition();
            trans.initialWave = ZombieTransitions.emptyComposition();
            trans.initialZombieCount = p => zombieAmount(p, 20);
            trans.initialTime = 30;
 
            trans.spawnComposition = ZombieTransitions.constComposition((new List<ZombieType>()).addType(ZombieTransitions.AlienZombie, 7).addType(ZombieTransitions.InfectedZombie, 4).addType(ZombieTransitions.SuicideZombie, 3));
            trans.finalZombieCount = p => zombieAmount(p, 12);
            trans.finalTime = 30;
 
 
            transitions.gracePeriod(30);
 
            //melee wave, we start to care if they're separated and/or/basing
            trans = transitions.newTransition();
            trans.initialWave = parameters => (new List<ZombieType>()).addType(ZombieTransitions.RangedZombie, 2).addType(ZombieTransitions.AlienZombie, zombieAmount(parameters, 6, (parameters.camp / 2000) + (parameters.separation / 2000)));
 
            transitions.gracePeriod(30);
 
            //a few ranged
            trans = transitions.newTransition();
            trans.spawnComposition = ZombieTransitions.constComposition((new List<ZombieType>()).addType(ZombieTransitions.AlienZombie, 5).addType(ZombieTransitions.HumanZombie, 3).addType(ZombieTransitions.DerangedZombie, 4).addType(ZombieTransitions.RangedZombie, 4));
            trans.finalZombieCount = parameters => zombieAmount(parameters, 3, (parameters.camp / 2000) + (parameters.separation / 2000));
            trans.initialZombieCount = parameters => zombieAmount(parameters, 4, (parameters.camp / 2000) + (parameters.separation / 2000)) / 2;
            ...
            //suicide rush
            trans = transitions.newTransition();
            trans.initialWave = parameters => (new List<ZombieType>()).addType(ZombieTransitions.InfectedZombie, 3).addType(ZombieTransitions.SuicideZombie, zombieAmount(parameters, 4, (parameters.camp / 1700) + (parameters.separation / 1700)));
 
            transitions.gracePeriod(25);
 
            //predators and suicides
            trans = transitions.newTransition();
            trans.spawnComposition = parameters => (new List<ZombieType>()).addType(ZombieTransitions.HumanZombie, 3).addType(ZombieTransitions.PredatorZombie, 3).addType(ZombieTransitions.SuicideZombie, 2).addType(ZombieTransitions.InfectedZombie, (parameters.camp / 2000) + (parameters.separation / 2000));
            trans.initialTime = 20;
            trans.finalTime = 50;
            trans.initialZombieCount = parameters => zombieAmount(parameters, 5, (parameters.camp / 1000) + (parameters.separation / 1000));
            trans.finalZombieCount = parameters => (int)(1.5 * zombieAmount(parameters, 4, (parameters.camp / 1000) + (parameters.separation / 1000)));
            ...
            List<ZombieType> deathWave1Composition = (new List<ZombieType>()).addType(ZombieTransitions.RepulsorZombie, 2).addType(ZombieTransitions.DerangedZombie, 2).addType(ZombieTransitions.SuicideZombie, 2).addType(ZombieTransitions.RangedZombie, 2).addType(ZombieTransitions.InfectedZombie, 1);
            ...
            trans = transitions.newTransition();
            trans.initialWave = parameters => (new List<ZombieType>()).addType(ZombieTransitions.AlienZombie, zombieAmount(parameters, 5)).addType(ZombieTransitions.HumanZombie, zombieAmount(parameters, 5)).addType(ZombieTransitions.InfectedZombie, zombieAmount(parameters, 5, (parameters.camp / 500) + (parameters.separation / 500)));
            trans.started = delegate(Script_ZombieZone.TeamState state)
            {
                state.team.sendArenaMessage("@The zombies are amassing! Prepare yourself for the ambush!");
            };
            ...
            trans.spawnComposition = parameters => (new List<ZombieType>()).addType(ZombieTransitions.HiveZombie, 6).addType(ZombieTransitions.AlienZombie, 1).addType(ZombieTransitions.DisruptorZombie, 3).addType(ZombieTransitions.RepulsorZombie, parameters.camp / 1000).addType(ZombieTransitions.InfectedZombie, parameters.separation / 850);
            trans.initialTime = 20;
            trans.finalTime = 40;
            trans.initialZombieCount = ZombieTransitions.constInt(5);
            trans.finalZombieCount = parameters => zombieAmount(parameters, 4, (parameters.camp / 1000) + (parameters.separation / 800));
            ...
            trans = transitions.newTransition();
            trans.spawnComposition = parameters => (new List<ZombieType>()).addType(ZombieTransitions.RepulsorZombie, 2 + (parameters.camp / 600)).addType(ZombieTransitions.PredatorZombie, 3).addType(ZombieTransitions.DerangedZombie, 3 + (parameters.separation / 800)).addType(ZombieTransitions.SuicideZombie, 2).addType(ZombieTransitions.DisruptorZombie, 3).addType(ZombieTransitions.InfectedZombie, 2 + (parameters.separation / 900));
            trans.finalTime = 50;
            trans.finalZombieCount = parameters => zombieAmount(parameters, 5, (parameters.camp / 1200) + (parameters.separation / 1000));
            ...
            trans.ended = delegate(Script_ZombieZone.TeamState state)
            {
                ...
                state.team.sendArenaMessage("The zombies seem to be regrouping. Get ready!");
            };
            ...
            //PHASE 2
            trans = transitions.newTransition();
            trans.spawnComposition = parameters => (new List<ZombieType>()).addType(ZombieTransitions.KamikazeZombie, 4).addType(ZombieTransitions.DerangedZombie, 2).addType(ZombieTransitions.DisruptorZombie, 3).addType(ZombieTransitions.HumanZombie, 4).addType(ZombieTransitions.AlienZombie, 1).addType(ZombieTransitions.AcidZombie, 1 + (parameters.camp / 700)).addType(ZombieTransitions.RepulsorZombie, parameters.camp / 1700);
            trans.finalTime = 40;
            ...
            trans = transitions.newTransition();
            trans.spawnComposition = parameters => (new List<ZombieType>()).addType(ZombieTransitions.KamikazeZombie, 1).addType(ZombieTransitions.AlienZombie, 1).addType(ZombieTransitions.DerangedZombie, 2 + (parameters.separation / 2000)).addType(ZombieTransitions.RangedZombie, 2 + (parameters.separation / 2000)).addType(ZombieTransitions.AcidZombie, 3 + (parameters.camp / 600)).addType(ZombieTransitions.AsgardianZombie, 2).addType(ZombieTransitions.RepulsorZombie, 2 + (parameters.camp / 500)).addType(ZombieTransitions.InfestedZombie, 2 + (parameters.separation / 900) * 2).addType(ZombieTransitions.HumanZombie, 1);
            trans.initialTime = 10;
            trans.finalTime = 40;
            trans.initialZombieCount = ZombieTransitions.constInt(1);
            trans.finalZombieCount = parameters => zombieAmount(parameters, 5, (parameters.camp / 1000) + (parameters.separation / 1000));
            ...
            //hive/acid swarm
            trans = transitions.newTransition();
            trans.spawnComposition = parameters => (new List<ZombieType>()).addType(ZombieTransitions.LairZombie, 6).addType(ZombieTransitions.AcidZombie, 4 + (parameters.camp < 3500 ? 2 : 0)).addType(ZombieTransitions.DoomZombie, parameters.camp < 3500 ? 1 : 0).addType(ZombieTransitions.RepulsorZombie, (parameters.camp / 1500));
            trans.initialTime = 10;
            trans.initialZombieCount = ZombieTransitions.constInt(1);
            trans.finalTime = 50;
            ...
            //plus a few ranged
            trans = transitions.newTransition();
            trans.spawnComposition = parameters => (new List<ZombieType>()).addType(ZombieTransitions.LairZombie, 5).addType(ZombieTransitions.PredatorZombie, 1).addType(ZombieTransitions.AcidZombie, 2 - (parameters.camp > 1500 ? 1 : 0)).addType(ZombieTransitions.DoomZombie, 1 + (parameters.camp > 1500 ? 1 : 0)).addType(ZombieTransitions.DerangedZombie, 3 - (parameters.camp > 2000 ? 1 : 0)).addType(ZombieTransitions.RangedZombie, 3 + (parameters.camp > 2000 ? 1 : 0)).addType(ZombieTransitions.DisruptorZombie, 2 + (parameters.camp > 2000 ? 1 : 0)).addType(ZombieTransitions.RepulsorZombie, 2 + (parameters.camp / 2000));
            trans.initialTime = 10;
            trans.initialZombieCount = ZombieTransitions.constInt(1);
            trans.finalTime = 50;
            trans.finalZombieCount = parameters => zombieAmount(parameters, 4, (parameters.camp / 900) + (parameters.separation / 800));
            trans.spawnRate = parameters => Math.Max(60, 1000 - 50 * parameters.playing - 50 * (parameters.camp / 1000) - 150 * (parameters.separation / 500) - 200 * parameters.vehicleClass);
            ...
            List<ZombieType> deathWave2Composition = (new List<ZombieType>()).addType(ZombieTransitions.RepulsorZombie, 2).addType(ZombieTransitions.DestroyerZombie, 3).addType(ZombieTransitions.DisruptorZombie, 1).addType(ZombieTransitions.KryptonianZombie, 2).addType(ZombieTransitions.RageZombie, 2).addType(ZombieTransitions.DoomZombie, 3).addType(ZombieTransitions.InfatuatedZombie, 2);
            ...
            trans = transitions.newTransition();
            trans.spawnComposition = parameters => (new List<ZombieType>()).addType(ZombieTransitions.LairZombie, parameters.separation / 1500).addType(ZombieTransitions.InfestedZombie, parameters.separation / 1750).addType(ZombieTransitions.AcidZombie, 1).addType(ZombieTransitions.AsgardianZombie, 3).addType(ZombieTransitions.DerangedZombie, 2).addType(ZombieTransitions.DoomZombie, 1).addType(ZombieTransitions.RepulsorZombie, 3 + (parameters.camp / 1000));
            trans.initialTime = 10;
            trans.finalTime = 50;
            trans.finalZombieCount = parameters => zombieAmount(parameters, 5, (parameters.camp / 1000) + (parameters.separation / 1000));
            trans.initialZombieCount = parameters => zombieAmount(parameters, 5, (parameters.camp / 1000) + (parameters.separation / 1000)) / 2;
            ...
            trans.started = delegate(Script_ZombieZone.TeamState state)
            {
                ...
                if (state.team.ActivePlayerCount <= 2)
                    state.team.sendArenaMessage("&Starting to wonder if anybody else is alive?");
            ...
            //infected attack
            trans = transitions.newTransition();
            trans.spawnComposition = parameters => (new List<ZombieType>()).addType(ZombieTransitions.InfatuatedZombie, 6 - parameters.camp / 714).addType(ZombieTransitions.InfatuatedZombie, parameters.camp / 714 + parameters.separation / 1000).addType(ZombieTransitions.DoomZombie, parameters.camp / 2500).addType(ZombieTransitions.DisruptorZombie, 2).addType(ZombieTransitions.RepulsorZombie, parameters.camp / 1400);
            trans.initialTime = 5;
            trans.initialZombieCount = ZombieTransitions.constInt(1);
            trans.finalTime = 45;
            trans.finalZombieCount = parameters => zombieAmount(parameters, 4, (parameters.camp / 800) + (parameters.separation / 700));
            ...
            //balanced, large army
            trans = transitions.newTransition();
            trans.spawnComposition = p => (new List<ZombieType>()).addType(ZombieTransitions.AcidZombie, 1 - (p.camp / 3600)).addType(ZombieTransitions.DoomZombie, p.camp / 3600).addType(ZombieTransitions.AsgardianZombie, 2 - (p.camp / 2700)).addType(ZombieTransitions.KryptonianZombie, p.camp / 2700).addType(ZombieTransitions.DisruptorZombie, 3).addType(ZombieTransitions.RepulsorZombie, 1 + (p.camp / 2000)).addType(ZombieTransitions.DerangedZombie, 2 - (p.camp / 2000) - (p.separation / 1500)).addType(ZombieTransitions.RageZombie, (p.camp / 2000) + (p.separation / 1500)).addType(ZombieTransitions.KamikazeZombie, 1 - (p.camp / 2000)).addType(ZombieTransitions.DestroyerZombie, p.camp / 2000);
            trans.initialTime = 10;
            trans.initialZombieCount = parameters => zombieAmount(parameters, 3, (parameters.camp / 2000) + (parameters.separation / 2000)) / 2;
            trans.finalTime = 50;
            trans.finalZombieCount = parameters => zombieAmount(parameters, 4, (parameters.camp / 2000) + (parameters.separation / 2000));
            ...
            trans.ended = delegate(Script_ZombieZone.TeamState state)
            {
                state.team.sendArenaMessage("That was close! Better keep steady and stay grouped in order to not get orverran.");
            };
            ...
            trans.initialZombieCount = parameters => zombieAmount(parameters, 4, (parameters.camp / 2000) + (parameters.separation / 2000)) / 2;
            trans.finalTime = 50;
            trans.finalZombieCount = parameters => zombieAmount(parameters, 5, (parameters.camp / 2000) + (parameters.separation / 2000));
            ...
            trans.ended = delegate(Script_ZombieZone.TeamState state)
            {
                state.team.sendArenaMessage("The sun is setting, and it seems like it is making the zombies stronger. Better keep your eyes open and reflexes sharp!", 1);
                state.zombieMessage("The night is will be ours!", 1);
            };
 
            transitions.gracePeriod(35);
 
            transitions.currentLevel = 3;
 
            //PHASE 3
            trans = transitions.newTransition();
            trans.spawnComposition = (p) => (new List<ZombieType>()).addType(ZombieTransitions.DoomZombie, 3).addType(ZombieTransitions.KryptonianZombie, 2).addType(ZombieTransitions.LairZombie, 2).addType(ZombieTransitions.DisruptorZombie, 2).addType(ZombieTransitions.RageZombie, 2).addType(ZombieTransitions.RepulsorZombie, p.camp / 1000).addType(ZombieTransitions.InfatuatedZombie, p.separation / 800);
            trans.initialTime = 15;
            trans.initialZombieCount = parameters => zombieAmount(parameters, 3, (parameters.camp / 1250) + (parameters.separation / 1250)) / 2;
            trans.finalTime = 40;
            trans.finalZombieCount = parameters => zombieAmount(parameters, 4, (parameters.camp / 1250) + (parameters.separation / 1250));
            trans.spawnRate = parameters => Math.Max(150, 1000 - 100 * parameters.playing - 90 * (parameters.camp / 1000) - 80 * (parameters.separation / 300) - 300 * parameters.vehicleClass);
 
            transitions.gracePeriod(20);
            transitions.gracePeriod(20).started = delegate(Script_ZombieZone.TeamState state)
            {
                state.team.sendArenaMessage("&These zombies are almost impossible to kill.  We have to try to outrun them!", 8);
                state.zombieMessage("&The Death Zombies are awakening!  Prepare to swarm the puny humans.", 8);
            };
 
            trans = transitions.newTransition();
            trans.spawnComposition = (p) => (new List<ZombieType>()).addType(ZombieTransitions.DeathZombie, 8).addType(ZombieTransitions.DoomZombie, 3).addType(ZombieTransitions.DerangedZombie, 2).addType(ZombieTransitions.InfatuatedZombie, 2 + p.separation / 1200);
            trans.initialTime = 15;
            ...
            {
                state.wipeOut();
                state.team.sendArenaMessage("What the hell just happened?  They disappeared! Now is the chance to regroup!");
                state.zombieMessage("The Death Zombies have burrowed. It won't be long until the opportune moment to re-emerge and overtake the survivors.");
            };
            ...
            trans = transitions.newTransition();
            trans.spawnComposition = (p) => (new List<ZombieType>()).addType(ZombieTransitions.KryptonianZombie, 1).addType(ZombieTransitions.DoomZombie, 1).addType(ZombieTransitions.DisruptorZombie, 3).addType(ZombieTransitions.RageZombie, 2).addType(ZombieTransitions.KamikazeZombie, 2).addType(ZombieTransitions.RepulsorZombie, p.camp / 1250).addType(ZombieTransitions.InfatuatedZombie, p.separation / 700).addType(ZombieTransitions.AsgardianZombie, 2);
            trans.initialTime = 15;
            trans.initialZombieCount = parameters => zombieAmount(parameters, 5, (parameters.camp / 1250) + (parameters.separation / 1250)) / 2;
            trans.finalTime = 50;
            trans.finalZombieCount = parameters => zombieAmount(parameters, 4, (parameters.camp / 1250) + (parameters.separation / 1250));
            trans.spawnRate = parameters => Math.Max(150, 1000 - 100 * parameters.playing - 90 * (parameters.camp / 1000) - 80 * (parameters.separation / 300) - 300 * parameters.vehicleClass);
            ...
            //aaerox wave
            trans = transitions.newTransition();
            trans.initialWave = p => (new List<ZombieType>()).addType(ZombieTransitions.DoomZombie, 1 * p.playing).addType(ZombieTransitions.RepulsorZombie, 2 * p.playing).addType(ZombieTransitions.DisruptorZombie, 2 * p.playing).addType(ZombieTransitions.DestroyerZombie, 1 * p.playing).addType(ZombieTransitions.RageZombie, 2 * p.playing);
            trans.spawnComposition = p => (new List<ZombieType>()).addType(ZombieTransitions.DoomZombie, 3).addType(ZombieTransitions.RepulsorZombie, 3).addType(ZombieTransitions.DisruptorZombie, 2 - (p.camp / 1200) + (p.separation / 800)).addType(ZombieTransitions.DestroyerZombie, 4).addType(ZombieTransitions.RageZombie, 3);
            trans.initialTime = 30;
            trans.initialZombieCount = ZombieTransitions.constInt(4);
            trans.finalTime = 50;
            trans.finalZombieCount = ZombieTransitions.constInt(5);
            trans.spawnRate = ZombieTransitions.constInt(50);
            trans.started = delegate(Script_ZombieZone.TeamState state)
            {
                state.team.sendArenaMessage("You must keep going! The zombies are forming another pack and beginning to rush us!");
                state.zombieMessage("The zombies ROSE for this!  Now, UNLEASH their true nature on those pathetic survivors and feast!");
            };
            ...
            trans = transitions.newTransition();
            trans.spawnComposition = parameters => (new List<ZombieType>()).addType(ZombieTransitions.AsgardianZombie, 2).addType(ZombieTransitions.DoomZombie, 2).addType(ZombieTransitions.DisruptorZombie, 3 + (parameters.separation / 800)).addType(ZombieTransitions.RageZombie, 2).addType(ZombieTransitions.KryptonianZombie, 1 + (parameters.camp / 1150)).addType(ZombieTransitions.KamikazeZombie, 1).addType(ZombieTransitions.InfestedZombie, 2 + (parameters.camp / 1000) + (parameters.separation / 750));
            trans.initialTime = 15;
            trans.finalTime = 50;
            trans.initialZombieCount = parameters => zombieAmount(parameters, 4, (parameters.camp / 1000) + (parameters.separation / 1000));
            trans.finalZombieCount = parameters => (int)(1.5 * zombieAmount(parameters, 4, (parameters.camp / 1000) + (parameters.separation / 1000)));
            trans.spawnRate = parameters => Math.Max(150, 1000 - 50 * parameters.playing - 100 * (parameters.camp / 1000) - 50 * (parameters.separation / 500) - 100 * parameters.vehicleClass);
            ...
            trans = transitions.newTransition();
            trans.spawnComposition = parameters => (new List<ZombieType>()).addType(ZombieTransitions.PredatorZombie, 1).addType(ZombieTransitions.RepulsorZombie, 2 + (parameters.camp / 1000) + (parameters.separation / 950)).addType(ZombieTransitions.DerangedZombie, 2).addType(ZombieTransitions.DoomZombie, 2).addType(ZombieTransitions.RageZombie, (parameters.camp / 900) + (parameters.separation / 1100));
            trans.initialTime = 20;
            trans.finalTime = 50;
            trans.initialZombieCount = parameters => zombieAmount(parameters, 4, (parameters.camp / 1000) + (parameters.separation / 1000));
            trans.finalZombieCount = parameters => (int)(1.5 * zombieAmount(parameters, 4, (parameters.camp / 1000) + (parameters.separation / 1000)));
            ...
            trans = transitions.newTransition();
            trans.spawnComposition = parameters => (new List<ZombieType>()).addType(ZombieTransitions.LairZombie, 6).addType(ZombieTransitions.DoomZombie, 3).addType(ZombieTransitions.DisruptorZombie, 3).addType(ZombieTransitions.KryptonianZombie, 1).addType(ZombieTransitions.RageZombie, 2).addType(ZombieTransitions.RepulsorZombie, 2 + (parameters.camp / 750) + (parameters.separation / 1000)).addType(ZombieTransitions.InfestedZombie, 2 + (parameters.camp / 1000) + (parameters.separation / 1500));
            trans.initialTime = 15;
            trans.finalTime = 50;
            trans.initialZombieCount = parameters => zombieAmount(parameters, 4, (parameters.camp / 1000) + (parameters.separation / 1000));
            trans.finalZombieCount = parameters => (int)(1.5 * zombieAmount(parameters, 4, (parameters.camp / 1000) + (parameters.separation / 1000)));
            ...
            trans.started = delegate(Script_ZombieZone.TeamState state)
            {
                state.team.sendArenaMessage("!Oh no!!");
            };
            ...
            //Phase 4
            trans = transitions.newTransition();
            trans.spawnComposition = parameters => (new List<ZombieType>()).addType(ZombieTransitions.PredatorZombie, 2).addType(ZombieTransitions.AlienZombie, 3).addType(ZombieTransitions.AcidZombie, 2).addType(ZombieTransitions.RepulsorZombie, 1 + (parameters.camp / 1200) + (parameters.camp / 1200)).addType(ZombieTransitions.DoomZombie, (parameters.camp / 1000) + (parameters.separation / 1700));
            trans.initialTime = 20;
            trans.finalTime = 50;
            trans.initialZombieCount = parameters => zombieAmount(parameters, 5, (parameters.camp / 1000) + (parameters.separation / 1000));
            trans.finalZombieCount = parameters => (int)(1.5 * zombieAmount(parameters, 5, (parameters.camp / 1000) + (parameters.separation / 1000)));
            trans.spawnRate = parameters => Math.Max(150, 1000 - 50 * parameters.playing - 100 * (parameters.camp / 1000) - 50 * (parameters.separation / 500) - 100 * parameters.vehicleClass);
 
            transitions.gracePeriod(35);
 
            trans = transitions.newTransition();
            trans.spawnComposition = parameters => (new List<ZombieType>()).addType(ZombieTransitions.HumanZombie, 3).addType(ZombieTransitions.AsgardianZombie, 2).addType(ZombieTransitions.KryptonianZombie, 1 + (parameters.camp / 1250)).addType(ZombieTransitions.InfectedZombie, 3 + (parameters.camp / 900) + (parameters.separation / 1400)).addType(ZombieTransitions.InfestedZombie, 2 + (parameters.camp / 1000) + (parameters.separation / 1500));
            trans.initialTime = 20;
            trans.finalTime = 50;
            trans.initialZombieCount = parameters => zombieAmount(parameters, 4, (parameters.camp / 1000) + (parameters.separation / 1000));
            trans.finalZombieCount = parameters => (int)(1.5 * zombieAmount(parameters, 5, (parameters.camp / 1000) + (parameters.separation / 1000)));
            ...
            trans = transitions.newTransition();
            trans.spawnComposition = parameters => (new List<ZombieType>()).addType(ZombieTransitions.SuicideZombie, 3).addType(ZombieTransitions.KamikazeZombie, 3).addType(ZombieTransitions.InfatuatedZombie, 2 + (parameters.camp / 1250) + (parameters.separation / 1000)).addType(ZombieTransitions.DestroyerZombie, 1).addType(ZombieTransitions.DerangedZombie, 2 + (parameters.camp / 1100) + (parameters.separation / 1050));
            trans.initialTime = 15;
            trans.finalTime = 50;
            trans.initialZombieCount = parameters => zombieAmount(parameters, 4, (parameters.camp / 1000) + (parameters.separation / 1000));
            trans.finalZombieCount = parameters => (int)(1.5 * zombieAmount(parameters, 4, (parameters.camp / 1000) + (parameters.separation / 1000)));
            ...
            trans.started = delegate(Script_ZombieZone.TeamState state)
            {
                state.team.sendArenaMessage("Suicide rush! This is where repulsion shields are very handy.");
            };
            ...
            trans = transitions.newTransition();
            trans.spawnComposition = parameters => (new List<ZombieType>()).addType(ZombieTransitions.LairZombie, 6).addType(ZombieTransitions.DoomZombie, 2).addType(ZombieTransitions.DerangedZombie, 2).addType(ZombieTransitions.RangedZombie, 3).addType(ZombieTransitions.RageZombie, 2).addType(ZombieTransitions.RepulsorZombie, 1 + (parameters.camp / 750) + (parameters.separation / 1000));
            trans.initialTime = 20;
            trans.finalTime = 50;
            trans.initialZombieCount = parameters => zombieAmount(parameters, 4, (parameters.camp / 1000) + (parameters.separation / 1000));
            trans.finalZombieCount = parameters => (int)(1.5 * zombieAmount(parameters, 5, (parameters.camp / 1000) + (parameters.separation / 1000)));
            ...
            trans = transitions.newTransition();
            trans.spawnComposition = parameters => (new List<ZombieType>()).addType(ZombieTransitions.DisruptorZombie, 1).addType(ZombieTransitions.RangedZombie, 1).addType(ZombieTransitions.HumanZombie, 2 + (parameters.camp / 1000) + (parameters.separation / 1250)).addType(ZombieTransitions.KryptonianZombie, 1).addType(ZombieTransitions.DerangedZombie, 2).addType(ZombieTransitions.RepulsorZombie, 1 + (parameters.camp / 750) + (parameters.separation / 1000)).addType(ZombieTransitions.InfatuatedZombie, 3);
            trans.initialTime = 15;
            trans.finalTime = 55;
            trans.initialZombieCount = parameters => zombieAmount(parameters, 5, (parameters.camp / 1000) + (parameters.separation / 1000));
            trans.finalZombieCount = parameters => (int)(1.5 * zombieAmount(parameters, 6, (parameters.camp / 1000) + (parameters.separation / 1000)));
            trans.spawnRate = parameters => Math.Max(150, 1000 - 50 * parameters.playing - 100 * (parameters.camp / 1000) - 50 * (parameters.separation / 1000) - 100 * parameters.vehicleClass);
 
            transitions.gracePeriod(25);
 
            trans = transitions.newTransition();
            trans.spawnComposition = p => (new List<ZombieType>()).addType(ZombieTransitions.RepulsorZombie, 2).addType(ZombieTransitions.AcidZombie, 1 + (p.camp / 3600)).addType(ZombieTransitions.AsgardianZombie, 2).addType(ZombieTransitions.InfatuatedZombie, 1 + (p.camp / 1700)).addType(ZombieTransitions.DisruptorZombie, 5 + (p.separation / 1500));
            trans.initialTime = 10;
            trans.initialZombieCount = parameters => zombieAmount(parameters, 5, (parameters.camp / 2000) + (parameters.separation / 2000)) / 2;
            trans.finalTime = 55;
            trans.finalZombieCount = parameters => zombieAmount(parameters, 5, (parameters.camp / 2000) + (parameters.separation / 2000));
            trans.spawnRate = parameters => Math.Max(150, 1000 - 100 * parameters.playing - 70 * (parameters.camp / 1000) - 100 * (parameters.separation / 300) - 300 * parameters.vehicleClass);
            ...
            trans = transitions.newTransition();
            trans.spawnComposition = p => (new List<ZombieType>()).addType(ZombieTransitions.PredatorZombie, 2).addType(ZombieTransitions.DoomZombie, 2 + (p.camp / 1500) + (p.separation / 1750)).addType(ZombieTransitions.HumanZombie, 3).addType(ZombieTransitions.KamikazeZombie, 1 + (p.camp / 1700)).addType(ZombieTransitions.RangedZombie, 2).addType(ZombieTransitions.RepulsorZombie, 1 + (p.camp / 2000));
            trans.initialTime = 20;
            trans.initialZombieCount = parameters => zombieAmount(parameters, 3, (parameters.camp / 2000) + (parameters.separation / 2000)) / 2;
            trans.finalTime = 50;
            trans.finalZombieCount = parameters => zombieAmount(parameters, 5, (parameters.camp / 2000) + (parameters.separation / 2000));
            ...
            trans.started = delegate(Script_ZombieZone.TeamState state)
            {
                state.team.sendArenaMessage("!...don't fear the reaper...");
            };
            ...
            //Phase 5
            trans = transitions.newTransition();
            trans.spawnComposition = p => (new List<ZombieType>()).addType(ZombieTransitions.AlienZombie, 3).addType(ZombieTransitions.HumanZombie, 2).addType(ZombieTransitions.AsgardianZombie, 1 + (p.camp / 1500) + (p.separation / 1750)).addType(ZombieTransitions.PredatorZombie, 1 + (p.camp / 2700)).addType(ZombieTransitions.InfatuatedZombie, 4 + (p.camp / 750) + (p.separation / 750));
            trans.initialTime = 10;
            trans.initialZombieCount = parameters => zombieAmount(parameters, 4, (parameters.camp / 2000) + (parameters.separation / 2000)) / 2;
            trans.finalTime = 50;
            trans.finalZombieCount = parameters => zombieAmount(parameters, 6, (parameters.camp / 2000) + (parameters.separation / 2000));
            trans.spawnRate = parameters => Math.Max(150, 1000 - 100 * parameters.playing - 70 * (parameters.camp / 1000) - 100 * (parameters.separation / 300) - 300 * parameters.vehicleClass);
 
            transitions.gracePeriod(25);
 
            trans = transitions.newTransition();
            trans.spawnComposition = p => (new List<ZombieType>()).addType(ZombieTransitions.DisruptorZombie, 4 + (p.camp / 700) + (p.separation / 500)).addType(ZombieTransitions.KryptonianZombie, 1 + (p.camp / 1700)).addType(ZombieTransitions.DerangedZombie, 2).addType(ZombieTransitions.RepulsorZombie, 1 + (p.camp / 1000) + (p.separation / 1250)).addType(ZombieTransitions.RageZombie, (p.camp / 1000) + (p.separation / 2000)).addType(ZombieTransitions.KamikazeZombie, 2 + (p.camp / 1500)).addType(ZombieTransitions.AcidZombie, 2);
            trans.initialTime = 10;
            trans.initialZombieCount = parameters => zombieAmount(parameters, 4, (parameters.camp / 2000) + (parameters.separation / 2000)) / 2;
            trans.finalTime = 50;
            trans.finalZombieCount = parameters => zombieAmount(parameters, 5, (parameters.camp / 2000) + (parameters.separation / 2000));
            ...
            trans.started = delegate(Script_ZombieZone.TeamState state)
            {
                state.team.sendArenaMessage("&Oh no! The Death Zombies are back!  We have to outrun them again!", 8);
                state.zombieMessage("&The Death Zombies are re-awakening!  Prepare to attack the survivors.", 8);
            };
 
            trans = transitions.newTransition();
            trans.spawnComposition = (p) => (new List<ZombieType>()).addType(ZombieTransitions.DeathZombie, 8).addType(ZombieTransitions.DoomZombie, 2).addType(ZombieTransitions.DerangedZombie, 3).addType(ZombieTransitions.AsgardianZombie, 1).addType(ZombieTransitions.InfatuatedZombie, 2 + p.separation / 1200);
            trans.initialTime = 15;
            trans.initialZombieCount = parameters => trans.finalZombieCount(parameters) / 3;
            trans.finalTime = 60;
            trans.finalZombieCount = parameters => zombieAmount(parameters, 4, (parameters.camp / 1250) + (parameters.separation / 1250));
            ...
            {
                state.wipeOut();
                state.team.sendArenaMessage("They disapeared again!  Now is the chance to regroup!");
                state.zombieMessage("The Death Zombies have burrowed again. It won't be long until the opportune moment to re-emerge and overtake the survivors.");
            };
            trans.zombieTickers[0] = info => String.Format("Something feels familiar about this.. {0}.  {1}", info.player._alias, info.timeString);
            trans.humanTickers[2] = info => String.Format("Inexorable Death Wave: {0}", info.timeString);
            ...
            trans = transitions.newTransition();
            trans.spawnComposition = (p) => (new List<ZombieType>()).addType(ZombieTransitions.AlienZombie, 3).addType(ZombieTransitions.HumanZombie, 2).addType(ZombieTransitions.LairZombie, 2).addType(ZombieTransitions.AsgardianZombie, 2).addType(ZombieTransitions.RageZombie, 1).addType(ZombieTransitions.RepulsorZombie, p.camp / 1000).addType(ZombieTransitions.InfatuatedZombie, p.separation / 800);
            trans.initialTime = 15;
            trans.initialZombieCount = parameters => zombieAmount(parameters, 4, (parameters.camp / 1250) + (parameters.separation / 1250)) / 2;
            trans.finalTime = 55;
            trans.finalZombieCount = parameters => zombieAmount(parameters, 5, (parameters.camp / 1250) + (parameters.separation / 1250));
            trans.spawnRate = parameters => Math.Max(150, 1000 - 100 * parameters.playing - 90 * (parameters.camp / 1000) - 80 * (parameters.separation / 300) - 300 * parameters.vehicleClass);
 
            transitions.gracePeriod(30);
 
            trans = transitions.newTransition();
            trans.spawnComposition = (p) => (new List<ZombieType>()).addType(ZombieTransitions.AcidZombie, 2).addType(ZombieTransitions.RangedZombie, 4).addType(ZombieTransitions.HumanZombie, 1).addType(ZombieTransitions.KryptonianZombie, 2 + (p.camp / 1200) + (p.separation / 1700));
            trans.initialTime = 15;
            trans.initialZombieCount = parameters => zombieAmount(parameters, 4, (parameters.camp / 1250) + (parameters.separation / 1250)) / 2;
            trans.finalTime = 55;
            trans.finalZombieCount = parameters => zombieAmount(parameters, 4, (parameters.camp / 1250) + (parameters.separation / 1250));
            ...
            trans = transitions.newTransition();
            trans.spawnComposition = (p) => (new List<ZombieType>()).addType(ZombieTransitions.AsgardianZombie, 3).addType(ZombieTransitions.InfectedZombie, 3).addType(ZombieTransitions.InfestedZombie, 1).addType(ZombieTransitions.RageZombie, 1 + (p.camp / 800) + (p.separation / 1500));
            trans.initialTime = 10;
            trans.initialZombieCount = parameters => zombieAmount(parameters, 4, (parameters.camp / 1250) + (parameters.separation / 1250)) / 2;
            trans.finalTime = 55;
            trans.finalZombieCount = parameters => zombieAmount(parameters, 5, (parameters.camp / 1250) + (parameters.separation / 1250));
            ...
            trans = transitions.newTransition();
            trans.spawnComposition = (p) => (new List<ZombieType>()).addType(ZombieTransitions.PredatorZombie, 2).addType(ZombieTransitions.RangedZombie, 2).addType(ZombieTransitions.HumanZombie, 2).addType(ZombieTransitions.AcidZombie, 3).addType(ZombieTransitions.KryptonianZombie, p.camp / 1000);
            trans.initialTime = 15;
            trans.initialZombieCount = parameters => zombieAmount(parameters, 4, (parameters.camp / 1250) + (parameters.separation / 1250)) / 2;
            trans.finalTime = 40;
            trans.finalZombieCount = parameters => zombieAmount(parameters, 4, (parameters.camp / 1250) + (parameters.separation / 1250));
            ...
            trans = transitions.newTransition();
            trans.spawnComposition = (p) => (new List<ZombieType>()).addType(ZombieTransitions.HumanZombie, 3).addType(ZombieTransitions.AcidZombie, 2).addType(ZombieTransitions.LairZombie, 1).addType(ZombieTransitions.RangedZombie, 2).addType(ZombieTransitions.RepulsorZombie, 1 + (p.camp / 1000) + (p.separation / 1300));
            trans.initialTime = 10;
            trans.initialZombieCount = parameters => zombieAmount(parameters, 4, (parameters.camp / 1250) + (parameters.separation / 1250)) / 2;
            trans.finalTime = 50;
            trans.finalZombieCount = parameters => zombieAmount(parameters, 5, (parameters.camp / 1250) + (parameters.separation / 1250));
            ...
            //Phase 6
            trans = transitions.newTransition();
            trans.spawnComposition = (p) => (new List<ZombieType>()).addType(ZombieTransitions.DoomZombie, 1).addType(ZombieTransitions.AlienZombie, 5).addType(ZombieTransitions.LairZombie, 2).addType(ZombieTransitions.DerangedZombie, 2).addType(ZombieTransitions.RageZombie, 1);
            trans.initialTime = 15;
            trans.initialZombieCount = parameters => zombieAmount(parameters, 4, (parameters.camp / 1250) + (parameters.separation / 1250)) / 2;
            trans.finalTime = 50;
            trans.finalZombieCount = parameters => zombieAmount(parameters, 4, (parameters.camp / 1250) + (parameters.separation / 1250));
            ...
            trans.started = delegate(Script_ZombieZone.TeamState state)
            {
                state.team.sendArenaMessage("&Stay grouped and help eachother! They are rushing with a strong group!", 8);
                state.zombieMessage("&Now is our chance to take the survivors down! Follow the rush in!", 8);
            };
 
            trans = transitions.newTransition();
            trans.spawnComposition = (p) => (new List<ZombieType>()).addType(ZombieTransitions.InfatuatedZombie, 7).addType(ZombieTransitions.DerangedZombie, 1).addType(ZombieTransitions.DisruptorZombie, 3 + p.separation / 1200);
            trans.initialTime = 10;
            trans.initialZombieCount = parameters => trans.finalZombieCount(parameters) / 4;
            trans.finalTime = 50;
            trans.finalZombieCount = parameters => zombieAmount(parameters, 5, (parameters.camp / 1050) + (parameters.separation / 1050));
            trans.spawnRate = parameters => Math.Max(150, 1000 - 100 * parameters.playing - 90 * (parameters.camp / 1000) - 80 * (parameters.separation / 300) - 300 * parameters.vehicleClass);
            trans.ended = delegate(Script_ZombieZone.TeamState state)
            {
                state.wipeOut();
                state.team.sendArenaMessage("Where'd they all go?! Move! Move! Move!");
                state.zombieMessage("They have defended against the flood rush. Prepare for another ambush!");
            };
            trans.zombieTickers[0] = info => String.Format("Something feels familiar about this.. {0}.  {1}", info.player._alias, info.timeString);
            trans.humanTickers[2] = info => String.Format("Sudden Death Wave: {0}", info.timeString);
 
            trans = transitions.newTransition();
            trans.spawnComposition = (p) => (new List<ZombieType>()).addType(ZombieTransitions.AlienZombie, 3).addType(ZombieTransitions.LairZombie, 3).addType(ZombieTransitions.AsgardianZombie, 1).addType(ZombieTransitions.RepulsorZombie, p.camp / 1000).addType(ZombieTransitions.InfectedZombie, p.separation / 800);
            trans.initialTime = 15;
            trans.initialZombieCount = parameters => zombieAmount(parameters, 5, (parameters.camp / 1250) + (parameters.separation / 1250)) / 2;
            trans.finalTime = 50;
            trans.finalZombieCount = parameters => zombieAmount(parameters, 6, (parameters.camp / 1250) + (parameters.separation / 1250));
            trans.spawnRate = parameters => Math.Max(150, 1000 - 100 * parameters.playing - 90 * (parameters.camp / 1000) - 80 * (parameters.separation / 300) - 300 * parameters.vehicleClass);
            ...
            trans = transitions.newTransition();
            trans.spawnComposition = (p) => (new List<ZombieType>()).addType(ZombieTransitions.DoomZombie, 1).addType(ZombieTransitions.AcidZombie, 2).addType(ZombieTransitions.LairZombie, 4).addType(ZombieTransitions.DerangedZombie, 1).addType(ZombieTransitions.RageZombie, 2 + (p.camp / 500) + (p.separation / 1000));
            trans.initialTime = 15;
            trans.initialZombieCount = parameters => zombieAmount(parameters, 4, (parameters.camp / 1250) + (parameters.separation / 1250)) / 2;
            trans.finalTime = 60;
            trans.finalZombieCount = parameters => zombieAmount(parameters, 4, (parameters.camp / 1250) + (parameters.separation / 1250));
            ...
            trans = transitions.newTransition();
            trans.spawnComposition = (p) => (new List<ZombieType>()).addType(ZombieTransitions.AcidZombie, 3).addType(ZombieTransitions.HumanZombie, 2).addType(ZombieTransitions.SuicideZombie, 2).addType(ZombieTransitions.AsgardianZombie, 2).addType(ZombieTransitions.RangedZombie, 1).addType(ZombieTransitions.RepulsorZombie, p.camp / 1000).addType(ZombieTransitions.DisruptorZombie, p.separation / 800);
            trans.initialTime = 10;
            trans.initialZombieCount = parameters => zombieAmount(parameters, 4, (parameters.camp / 1250) + (parameters.separation / 1250)) / 2;
            trans.finalTime = 50;
            trans.finalZombieCount = parameters => zombieAmount(parameters, 4, (parameters.camp / 1250) + (parameters.separation / 1250));
            ...
            trans = transitions.newTransition();
            trans.spawnComposition = (p) => (new List<ZombieType>()).addType(ZombieTransitions.PredatorZombie, 2).addType(ZombieTransitions.SuicideZombie, 3).addType(ZombieTransitions.InfectedZombie, 4 + (p.camp / 550) + (p.separation / 450)).addType(ZombieTransitions.DerangedZombie, 1 + (p.camp / 700) + (p.separation / 400)).addType(ZombieTransitions.HumanZombie, 2).addType(ZombieTransitions.DisruptorZombie, 3 + (p.camp / 1000) + (p.separation / 500));
            trans.initialTime = 15;
            trans.initialZombieCount = parameters => zombieAmount(parameters, 6, (parameters.camp / 1250) + (parameters.separation / 1250)) / 2;
            trans.finalTime = 50;
            trans.finalZombieCount = parameters => zombieAmount(parameters, 6, (parameters.camp / 1250) + (parameters.separation / 1250));
            trans.spawnRate = parameters => Math.Max(150, 1000 - 100 * parameters.playing - 90 * (parameters.camp / 1000) - 80 * (parameters.separation / 300) - 300 * parameters.vehicleClass);
            ...
            trans = transitions.newTransition();
            trans.spawnComposition = (p) => (new List<ZombieType>()).addType(ZombieTransitions.DoomZombie, 1).addType(ZombieTransitions.KamikazeZombie, 2 + (p.camp / 500) + (p.separation / 1200)).addType(ZombieTransitions.SuicideZombie, 1 + (p.camp / 500) + (p.separation / 500)).addType(ZombieTransitions.RepulsorZombie, 2 + (p.camp / 500) + (p.separation / 800)).addType(ZombieTransitions.AlienZombie, 3).addType(ZombieTransitions.LairZombie, 2).addType(ZombieTransitions.DerangedZombie, 2).addType(ZombieTransitions.KryptonianZombie, 1 + (p.camp / 750)).addType(ZombieTransitions.HumanZombie, 1 + (p.camp / 500) + (p.separation / 750)).addType(ZombieTransitions.RageZombie, 1);
            trans.initialTime = 10;
            trans.initialZombieCount = parameters => zombieAmount(parameters, 5, (parameters.camp / 1250) + (parameters.separation / 1250)) / 2;
            trans.finalTime = 60;
            trans.finalZombieCount = parameters => zombieAmount(parameters, 5, (parameters.camp / 1250) + (parameters.separation / 1250));
            ...
            transitions.gracePeriod(15);
            trans = transitions.newTransition();
            trans.spawnComposition = (p) => (new List<ZombieType>()).addType(ZombieTransitions.PredatorZombie, 2).addType(ZombieTransitions.RangedZombie, 2).addType(ZombieTransitions.HumanZombie, 2).addType(ZombieTransitions.AcidZombie, 3).addType(ZombieTransitions.KryptonianZombie, 2);
            trans.initialTime = 10;
            trans.initialZombieCount = parameters => zombieAmount(parameters, 6, (parameters.camp / 1250) + (parameters.separation / 1250)) / 2;
            trans.finalTime = 50;
            trans.finalZombieCount = parameters => zombieAmount(parameters, 5, (parameters.camp / 1250) + (parameters.separation / 1250));
            ...
            trans = transitions.newTransition();
            trans.spawnComposition = (p) => (new List<ZombieType>()).addType(ZombieTransitions.AlienZombie, 3).addType(ZombieTransitions.AcidZombie, 2).addType(ZombieTransitions.LairZombie, 4).addType(ZombieTransitions.DerangedZombie, 1).addType(ZombieTransitions.InfectedZombie, 3).addType(ZombieTransitions.RageZombie, 2 + (p.camp / 500) + (p.separation / 1000));
            trans.initialTime = 15;
            trans.initialZombieCount = parameters => zombieAmount(parameters, 5, (parameters.camp / 1250) + (parameters.separation / 1250)) / 2;
            trans.finalTime = 60;
            trans.finalZombieCount = parameters => zombieAmount(parameters, 5, (parameters.camp / 1250) + (parameters.separation / 1250));
            ...
            trans = transitions.newTransition();
            trans.spawnComposition = (p) => (new List<ZombieType>()).addType(ZombieTransitions.AcidZombie, 3).addType(ZombieTransitions.HumanZombie, 2).addType(ZombieTransitions.SuicideZombie, 2).addType(ZombieTransitions.AsgardianZombie, 2).addType(ZombieTransitions.DisruptorZombie, 2).addType(ZombieTransitions.RepulsorZombie, p.camp / 1000).addType(ZombieTransitions.DisruptorZombie, p.separation / 800);
            trans.initialTime = 10;
            trans.initialZombieCount = parameters => zombieAmount(parameters, 5, (parameters.camp / 1250) + (parameters.separation / 1250)) / 2;
            trans.finalTime = 50;
            trans.finalZombieCount = parameters => zombieAmount(parameters, 5, (parameters.camp / 1250) + (parameters.separation / 1250));
            ...
            trans = transitions.newTransition();
            trans.spawnComposition = (p) => (new List<ZombieType>()).addType(ZombieTransitions.DoomZombie, 1).addType(ZombieTransitions.KamikazeZombie, 2 + (p.camp / 500) + (p.separation / 1200)).addType(ZombieTransitions.SuicideZombie, 1 + (p.camp / 500) + (p.separation / 500)).addType(ZombieTransitions.RepulsorZombie, 2 + (p.camp / 500) + (p.separation / 800)).addType(ZombieTransitions.AlienZombie, 3).addType(ZombieTransitions.LairZombie, 2).addType(ZombieTransitions.DerangedZombie, 2).addType(ZombieTransitions.KryptonianZombie, 1 + (p.camp / 750)).addType(ZombieTransitions.HumanZombie, 1 + (p.camp / 500) + (p.separation / 750)).addType(ZombieTransitions.DisruptorZombie, 2).addType(ZombieTransitions.RageZombie, 1);
            trans.initialTime = 10;
            trans.initialZombieCount = parameters => zombieAmount(parameters, 7, (parameters.camp / 1250) + (parameters.separation / 1250)) / 2;
            trans.finalTime = 60;
            trans.finalZombieCount = parameters => zombieAmount(parameters, 7, (parameters.camp / 1250) + (parameters.separation / 1250));
            trans.spawnRate = parameters => Math.Max(150, 1000 - 100 * parameters.playing - 90 * (parameters.camp / 1000) - 80 * (parameters.separation / 300) - 300 * parameters.vehicleClass);
