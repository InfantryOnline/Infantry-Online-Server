                ...
                switch (request.HttpMethod)
                {
                    ...
                    case "GET":
                        ...
                        if (request.Url.AbsolutePath.Contains("verify-token"))
                        {
                            ...
                            if (!client.IsResetTokenValid(tokenData))
                            {
                            ...
                            if (client.HasTokenExpired(tokenData))
                            {
                                responseString = Encoding.UTF8.GetBytes("false");
                            ...
                            if (client.WasTokenUsed(tokenData))
                            {
                                responseString = Encoding.UTF8.GetBytes("false");
                    ...
                    case "PUT":
                        ...
                        var regModel = JsonSerializer.Deserialize<Account.RegistrationRequestModel>(registrationData);
                        // 2. Data valid?
                    ...
                    case "POST":
                        ...
                        if (request.Url.AbsolutePath.Contains("recover"))
                        {
                            ...
                            var resetModel = JsonSerializer.Deserialize<Account.RecoverRequestModel>(resetData);
                            ...
                            if (!resetModel.Reset)
                            {
                                ...
                                string username;
                                if (!client.TryGetUsernameFromEmail(resetModel.Email, out username))
                                {
                                    response.StatusCode = 500; //Internal Server Error
                            ...
                            else
                            {
                                ...
                                if (!client.TryGenerateTokenForAccountReset(resetModel.Username, out email, out token))
                                {
                                    response.StatusCode = 500; //Internal Server Error
                                ...
                                // 4b. Was it successful?
                                if (!client.AccountSendMail(email, resetModel.Username, token))
                                {
                                    response.StatusCode = 500; //Internal Server Error
                                ...
                                emailResponse = client.EncodeEmail(email);
                            }
                        ...
                        {
                            string resetData = new StreamReader(request.InputStream).ReadToEnd();
                            var resetModel = JsonSerializer.Deserialize<Account.ResetRequestModel>(resetData);
                            ...
                            if (!client.IsResetTokenValid(resetModel.Token))
                            {
                                responseString = Encoding.UTF8.GetBytes("false");
                                response.StatusCode = 404; //Not Found
                        ...
                        var loginModel = JsonSerializer.Deserialize<Account.LoginRequestModel>(loginData);
                        // 2. Is the data valid?
                        ...
                        var loginResponseString = JsonSerializer.Serialize(loginResponseModel);
